...
class BlockchainFraudDetector:
    ...
    def visualize_results(self, results_df, output_file="fraud_analysis.png"):
        """
        ...
        """
        if 'fraud_probability' not in results_df.columns:
            logger.error("❌ No fraud_probability column")
            return
        
        fig, axes = plt.subplots(2, 2, figsize=(14, 10))
        fig.suptitle('🚨 Blockchain Fraud Detection Analysis', fontsize=16, fontweight='bold')
        
        # Plot 1: Distribution of fraud probabilities
        axes[0, 0].hist(results_df['fraud_probability'], bins=50, color='skyblue', edgecolor='black')
        ...
        color_list = [colors.get(risk, 'gray') for risk in risk_counts.index]
        axes[0, 1].bar(risk_counts.index, risk_counts.values, color=color_list)
        axes[0, 1].set_title('Transaction Risk Distribution')
        axes[0, 1].set_ylabel('Count')
        axes[0, 1].tick_params(axis='x', rotation=45)
        
        # Plot 3: Value vs Fraud Probability
        axes[1, 0].scatter(results_df['value_eth'], results_df['fraud_probability'], 
                          alpha=0.6, c=results_df['is_fraud'], cmap='RdYlGn_r')
        axes[1, 0].set_xlabel('Transaction Value (ETH)')
        axes[1, 0].set_ylabel('Fraud Probability')
        axes[1, 0].set_title('Value vs Fraud Risk')
        
        # Plot 4: Gas Price vs Fraud Probability
        axes[1, 1].scatter(results_df['gas_price_gwei'], results_df['fraud_probability'], 
                          alpha=0.6, c=results_df['is_fraud'], cmap='RdYlGn_r')
        axes[1, 1].set_xlabel('Gas Price (Gwei)')
        axes[1, 1].set_ylabel('Fraud Probability')
        axes[1, 1].set_title('Gas Price vs Fraud Risk')
